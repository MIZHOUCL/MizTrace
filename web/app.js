...
createApp({
  ...
  setup() {
    ...
    const error = ref('');
    const modules = ref([]);
    const notes = ref({ entries: [], images: [] });
    ...
    async function loadDay({ refresh = false } = {}) {
      ...
      try {
        ...
        notes.value = d.notes ?? { entries: [], images: [] };
    ...
    /** 保存一条手记：它随即成为时间板上的一个泡泡（loadDay 会把模块重建一遍）。 */
    ...
    /** 删掉某条手记（在它的泡泡里点「删掉这条手记」）：条目与它名下的图一起消失。 */
    async function deleteNote(id) {
      if (!id) return;
      if (!confirm(t('noteDeleteConfirm'))) return;
      saving.value = true;
      try {
        const r = await api('POST', '/api/day/note/delete', { date: date.value, id });
        notes.value = r.notes;
        drawer.value = null;
        await loadDay();
    ...
    return {
      ...
      rulesJournal, openPreview, confirmWrite, saveDraft, copyDraft, openSettings, testAi, saveSettings, finishSetup, skipSetup, onMdClick, installShell, saveNote, deleteNote, uploadImages, removeImage,
      setView: (v) => {
        view.value = v;
...
<div class="app">
  ...
  <template v-else>
    ...
    <div class="stage" :class="{withdrawer: drawer}">
      ...
      <drawer v-if="drawer" :module="drawer" :date="date" :saving="hintSaving" :vision="state.config && state.config.ai && state.config.ai.vision === true" @close="drawer = null" @pop="pop" @exclude="excludeProject" @hint="saveHint" @upload="uploadImages" @remove-image="removeImage" @delete-note="deleteNote"></drawer>
    </div>
