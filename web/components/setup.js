...
export const SetupPanel = {
  ...
  setup(props, { emit }) {
    ...
    const svnOn = ref(false);
    const svnRemote = ref(true);
    ...
    async function done() {
      ...
      try {
        emit('done', {
          ...
          svn: { enabled: svnOn.value, remote: svnRemote.value },
          setupDone: true,
        });
      } finally {
        saving.value = false;
    ...
    return { picked, extra, outline, replies, browsers, shellOn, svnOn, svnRemote, svnCount, svnReady, shellNames, picking, pickedDir, saving, err, roots, readableBrowsers, deniedBrowsers, foundProviders, supportedCount, anyBrowser, short, coveredBy, done, t, L, lang };
  },
  template: `
<section class="setup" aria-label="setup">
  ...
  <div class="setup-grid">
    ...
    <section class="block">
      ...
      <label class="toggle"><input type="checkbox" v-model="svnOn" :disabled="!svnCount || !svnReady"> <span>{{ L('SVN 工作副本', 'SVN working copies') }}<small v-if="!svnCount">{{ L('扫描目录下没有检测到 SVN 工作副本。', 'No SVN working copy found under your scan folders.') }}</small><small v-else-if="!svnReady">{{ L('找到 ' + svnCount + ' 个工作副本，但这台电脑上没有 svn 命令，装上才能采集。', svnCount + ' working copy/copies found, but the svn command is not installed.') }}</small><small v-else>{{ L('找到 ' + svnCount + ' 个工作副本。默认关闭：svn log 是向版本库服务器发起的查询，会联网。', svnCount + ' working copy/copies found. Off by default: svn log queries the repository server.') }}</small></span></label>
      <div class="indent" v-if="svnOn">
        <label class="toggle"><input type="checkbox" v-model="svnRemote"> <span>{{ L('读取提交记录', 'Read commit history') }}<small>{{ L('关掉只记录本地未提交改动，完全不联网；提交记录必须问服务器才能拿到。', 'Turn off to record only local uncommitted changes, fully offline; commit history requires the server.') }}</small></span></label>
      </div>
    </section>
  </div>
