...
export const SettingsModal = {
  ...
  setup(props, { emit }) {
    ...
    const f = reactive({
      ...
      svnOn: c.svn?.enabled === true,
      svnRemote: c.svn?.remote !== false,
    ...
    const svnCount = computed(() => (props.sources?.svn?.workingCopies ?? []).length);
    const svnReady = computed(() => props.sources?.svn?.available === true);
    watch(
      () => f.browserOn,
      (on) => {
    ...
    function payload() {
      ...
      return {
        ...
        svn: { enabled: f.svnOn, remote: f.svnRemote },
        ai: aiPayload(),
        setupDone: true,
      };
    }
    const keyPlaceholder = computed(() => (c.ai?.hasApiKey ? L(`已保存（${c.ai.apiKey}），留空不改`, `Saved (${c.ai.apiKey}); leave empty to keep`) : 'sk-…'));
    return { f, shell, showSnippet, picking, pickedDir, templates, editingTpl, currentTpl, resetTemplate, addTemplate, removeTemplate, copySnippet, readableBrowsers, deniedBrowsers, providers, svnCount, svnReady, keyPlaceholder, t, L, save: () => emit('save', payload()), test: () => emit('test', aiPayload()), installShell: () => emit('install-shell', []) };
  },
  template: `
  ...
  <div class="modal wide">
    ...
    <div class="form">
      ...
      <div class="toggles">
        ...
        <label class="toggle"><input type="checkbox" v-model="f.svnOn" :disabled="!svnCount || !svnReady"> <span>{{ L('SVN 工作副本', 'SVN working copies') }}<small v-if="!svnCount">{{ L('扫描目录下没有检测到 SVN 工作副本。', 'No SVN working copy found under your scan folders.') }}</small><small v-else-if="!svnReady">{{ L('找到 ' + svnCount + ' 个工作副本，但这台电脑上没有 svn 命令，装上才能采集。', svnCount + ' working copy/copies found, but the svn command is not installed.') }}</small><small v-else>{{ L('找到 ' + svnCount + ' 个工作副本。默认关闭：svn log 是向版本库服务器发起的查询，会联网。', svnCount + ' working copy/copies found. Off by default: svn log queries the repository server.') }}</small></span></label>
        <div class="indent" v-if="f.svnOn">
          <label class="toggle"><input type="checkbox" v-model="f.svnRemote"> <span>{{ L('读取提交记录', 'Read commit history') }}<small>{{ L('关掉只记录本地未提交改动，完全不联网；提交记录必须问服务器才能拿到。', 'Turn off to record only local uncommitted changes, fully offline; commit history requires the server.') }}</small></span></label>
          <p class="hint">{{ L('只读 revision、作者、时间、提交说明与改动路径，不读 diff 正文。', 'Only revision, author, time, commit message and changed paths — never diff bodies.') }}</p>
        </div>
      </div>
    </div>
