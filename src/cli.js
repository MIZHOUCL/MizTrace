...
用法
  ...
  miztrace show <source_id>        查看某条证据（如 commit:a9c7471、svn-commit:r814@a1b2c3d4）
  miztrace sources                 本机能读到哪些来源：AI 工具、浏览器、终端钩子、版本库、推荐的扫描目录
...
选项
  ...
  --no-files        关闭文件系统扫描（只看版本库与 AI 会话）
  --no-browser      这一次不读浏览器历史（配置里开着也不读）
  --no-shell        这一次不读终端命令
  --no-svn          这一次不采集 SVN 工作副本（配置里开着也不采）
...
const OPTIONS = {
  ...
  'no-svn': { type: 'boolean', default: false },
...
function sources(cfg, flags) {
  ...
  // SVN 工作副本的发现是纯文件系统遍历，不需要 svn 命令，所以没装 svn 也能告诉用户「你这儿有工作副本」
  const svnState = cfg.svn?.enabled ? (cfg.svn?.remote === false ? '已开启（只读本地改动，不联网）' : '已开启（svn log 会访问版本库服务器）') : '未开启';
  const workingCopies = findWorkingCopies(cfg.roots, cfg.svn?.maxDepth ?? 4);
  if (flags.json) {
    process.stdout.write(
      `${JSON.stringify({ roots: cfg.roots, rootsConfigured: cfg.rootsConfigured, providers, browsers, shell, svn: { available: svnAvailable(), enabled: cfg.svn?.enabled === true, remote: cfg.svn?.remote !== false, workingCopies }, candidates, disclosure: disclosure(cfg) }, null, 2)}\n`,
    );
    return 0;
  }
  const lines = [];
  ...
  lines.push('', `版本库：git 始终采集；SVN ${svnState}`);
  if (!workingCopies.length) {
    lines.push('  没有找到 SVN 工作副本');
  } else {
    lines.push(`  ${svnAvailable() ? '●' : '○'} SVN 工作副本 ${workingCopies.length} 个${svnAvailable() ? '' : '（但没找到 svn 命令，装上才能采集）'}：`);
    for (const wc of workingCopies.slice(0, 8)) lines.push(`    ${wc}`);
    if (workingCopies.length > 8) lines.push(`    …… 等 ${workingCopies.length} 个`);
    if (!cfg.svn?.enabled) lines.push('  要采集就把 config.json 的 svn.enabled 改成 true；注意 svn log 是向版本库服务器发起的查询。');
  }
  lines.push('', '推荐的扫描目录：');
