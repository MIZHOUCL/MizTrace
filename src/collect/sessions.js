...
/**
 * CodeBuddy Code 是照着 Claude Code 做的 CLI，WorkBuddy 是同一团队的桌面版（官网就跳到 codebuddy.cn/work）。
 * 两个都没拿到样例：先按 Claude Code 的 jsonl 格式读（sessionId + type + timestamp + message），一个会话都认不出再退回通用格式。
 * 读到的仍标「通用格式」—— 没核对过真实文件就不能说「专用」。目录遍历限深度和数量：桌面应用的数据目录可能很大。
 */
...
export const PROVIDERS = [
  ...
  { id: 'workbuddy', name: 'WorkBuddy', mod: claudeLike('workbuddy'), generic: true },
