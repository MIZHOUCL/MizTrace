...
/** 从一条消息的 content 里取纯文本：字符串直接用，数组只取 text 类块。 */
export function textOf(content) {
  if (typeof content === 'string') return content;
  if (Array.isArray(content)) {
    const parts = content.map((b) => (typeof b === 'string' ? b : b && typeof b.text === 'string' && (b.type === 'text' || b.type == null) ? b.text : null)).filter(Boolean);
