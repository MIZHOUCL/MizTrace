...
/**
 * 遍历时跳过的目录：体积黑洞、系统垃圾与无意义目录。
 *
 * 系统垃圾这几项是实查出来的（2026-09-15）：有人把扫描目录配成了整个盘（`C:\`、`D:\`、`E:\`），
 * 而 `C:\$Recycle.Bin\<SID>\$RXXXXXX` 只到第三层，远在 maxDepth 之内 ——
 * 于是代码把**回收站里**的 git 仓库当成了「今天新建的仓库」，一次刷出 45 个假项目。
 * 任何被删进回收站的项目文件夹都会重演这一幕，所以这些目录必须无条件跳过。
 */
export const SKIP_DIRS = new Set([
  ...
  // Windows：回收站、系统还原点、卷影副本、升级残留、Office 安装缓存、chkdsk 碎片
  '$Recycle.Bin', '$RECYCLE.BIN', 'System Volume Information', 'Recovery', 'Config.Msi',
  '$WINDOWS.~BT', '$WINDOWS.~WS', '$WinREAgent', 'MSOCache', 'found.000',
  // macOS / Linux：回收站、索引与版本目录、丢失碎片
  'Trash', 'lost+found', '.Spotlight-V100', '.fseventsd', '.DocumentRevisions-V100',
]);

/**
 * 目录名要不要跳过。
 * 「以点开头」天然大小写无关；其余要显式比 —— Windows 上同一个目录的写法并不固定
 * （回收站就同时存在 `$Recycle.Bin` 与 `$RECYCLE.BIN`），所以统一转小写再比。
 */
const SKIP_LOWER = new Set([...SKIP_DIRS].map((n) => n.toLowerCase()));
export function shouldSkipDir(name) {
  const n = String(name ?? '');
  return n.startsWith('.') || SKIP_LOWER.has(n.toLowerCase());
}

/** 一次采集最多认多少个仓库。整盘扫描下仓库可能是成百上千个、绝大多数是噪声，到顶就停并告警。 */
 ...
 * @param {string[]} [warnings] 传了就把「仓库太多、已截断」写进去，由调用方打印
 * @returns {string[]} 仓库根路径
 */
export function findRepos(roots, maxDepth = 4, warnings) {
  const found = new Set();
  let truncated = false;
  ...
  if (truncated && warnings) {
    warnings.push(
      `扫描到的 git 仓库超过 ${MAX_REPOS} 个，只保留前 ${MAX_REPOS} 个 —— 扫描目录（--root）多半开得太大了，指向真正写代码的目录会准得多`,
    );
  }
  return [...found].sort();
}
