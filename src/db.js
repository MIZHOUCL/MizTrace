...
/**
 * 手记的存储形态（notes_json）：
 *   { entries: [{ id, text, ts }], images: [{ id, name, mime, path, bytes, ts, entryId?, forKey? }] }
 *
 * - `entries`：**已保存**的手记。一条手记就是时间板上的一个泡泡，所以是多条，不是一天一条。
 * - `images` 里 `entryId` 有值 = 这张图属于那条手记；没有 entryId = 还留在输入框里、没保存。
 * - `forKey` 有值 = 给某个泡泡补的图，和手记是两回事。
 *
 * 早期版本是「一天一条」的 `{ text, updatedAt, images }`。parseNotes 会把老数据整条迁成第一条手记，
 * 文字与图片一张都不丢（图片补上 entryId）—— 用户升级后打开旧的一天，内容还在原处。
 */
export const EMPTY_NOTES = Object.freeze({ entries: [], images: [] });

/** 一天最多手记多少条：够用，又能挡住脚本刷。 */
export const MAX_NOTE_ENTRIES = 50;

const cleanImages = (images) => (Array.isArray(images) ? images.filter((i) => i && typeof i.id === 'string') : []);

/** 把任意来源的手记对象收敛成合法形态（长度、条数、字段类型都夹一遍）。 */
export function normalizeNotes(input) {
  ...
  const entries = (Array.isArray(src.entries) ? src.entries : [])
    ...
    .map((e) => ({
      id: e.id.slice(0, 40),
      text: String(e.text ?? '').slice(0, 20_000),
      ts: typeof e.ts === 'string' ? e.ts : null,
    }));
  const ids = new Set(entries.map((e) => e.id));
  const images = cleanImages(src.images).map((i) => {
    const out = { ...i };
    // 指向不存在的手记 = 图还在输入框里（那条手记可能刚被删掉），去掉 entryId 免得成为孤儿
    if (out.entryId && !ids.has(out.entryId)) delete out.entryId;
    return out;
  });
...
function parseNotes(raw) {
  try {
    ...
    if (!n || typeof n !== 'object') return { entries: [], images: [] };
    if (Array.isArray(n.entries)) return normalizeNotes(n);
    // 旧格式「一天一条」→ 迁成第一条手记
    const images = cleanImages(n.images);
    const text = typeof n.text === 'string' ? n.text : '';
    const ts = typeof n.updatedAt === 'string' ? n.updatedAt : null;
    if (!text.trim() && !images.some((i) => !i.forKey)) return { entries: [], images };
    const id = 'n0';
    return { entries: [{ id, text, ts }], images: images.map((i) => (i.forKey ? i : { ...i, entryId: id })) };
  } catch {
    return { entries: [], images: [] };
  }
}
