...
import { saveConfig, maskConfig, mergeAiConfig, mergeBrowserConfig, mergeShellConfig, dbPath, aiProblems, dataDir, configPath, dedupeRoots } from '../config.js';
import { dayRange, todayLocalDate, utcOffsetLabel, hhmm } from '../time.js';
import { openDb, getDayState, setOverrides, setHint, setNotes, setDraft, logAiRun, aiUsageToday } from '../db.js';
...
function publicNotes(notes) {
  return { text: notes?.text ?? '', updatedAt: notes?.updatedAt ?? null, images: (notes?.images ?? []).map((i) => ({ id: i.id, name: i.name, mime: i.mime, bytes: i.bytes, ts: i.ts, forKey: i.forKey ?? null })) };
}
...
export async function startServer(opts) {
  ...
  const api = {
    ...
    async 'POST /api/day/note'(q, body) {
      ...
      const next = setNotes(db, localDate, { ...notes, text, updatedAt: text ? new Date().toISOString() : notes.updatedAt });
