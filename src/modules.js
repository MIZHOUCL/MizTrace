 ...
 *
 * 证据种类（item.kind）：
 *   commit / worktree / file / action-file  —— 版本库与文件层面的痕迹（file 与 worktree 可能带 outline：文档大纲）
 *     其中 commit / worktree 同时承载 git 与 SVN 两种来源：kind 一样，证据 id 的类型不同
 *     （commit / svn-commit、worktree / svn-worktree），脚注据此区分措辞。
 ...
 *   note / image                            —— 你自己写的手记、配的图片（**一条手记一个模块**，权重最高，见 notesModules）
...
export function timelineOf(project, input) {
  ...
  for (const r of input.gitByProject.get(project.id) ?? []) {
    ...
    for (const c of r.commits) {
      // kind 仍是 'commit'（模块、日记、模型提示词都按这个 kind 走），
      // 只有证据 id 的类型区分 vcs，脚注才能写对「提交」还是「SVN r814」
      items.push({ ts: c.committedAt, kind: 'commit', label: c.message, sourceId: sourceId(c.sourceType ?? (isSvn ? 'svn-commit' : 'commit'), c.hash), files: c.files.length, revision: c.revision });
    }
    for (const d of r.dirty) {
      // 有真实 mtime 就用它，拿不到（文件已删）才退回「现在」
      items.push({ ts: d.mtime ?? input.nowIso, kind: 'worktree', label: d.path, sourceId: sourceId(d.sourceType ?? (isSvn ? 'svn-worktree' : 'worktree'), `${project.id}:${d.path}`), outline: d.outline?.items, outlineKind: d.outline?.kind });
    }
  }
...
/** 手记模块的 key。一条手记一个 key，这样它的「补充说明」和配图只挂在它自己身上。 */
export function noteKey(entryId) {
  return `note@${entryId}`;
}

/**
 * 手记 → 模块。**一条手记一个模块**，在时间板上就是一个泡泡；用户亲手写的最可信，权重压过一切。
 *
 * 为什么不是「一天一个模块」：一天里会写好几件不同的事，挤进一个框就分不出哪段是哪段，
 * 也没法单独戳掉某一条。key 用 `note@<条目 id>`，后写的第二条不会顶掉第一条的设置。
 * 没文字也没配图的条目不产生泡泡；一条都没有就没有手记模块。
 * @param {{entries?:{id:string,text:string,ts?:string|null}[], images?:any[]}} notes
 */
export function notesModules(notes, localDate) {
  const fallback = `${localDate}T12:00:00.000Z`;
  const all = Array.isArray(notes?.images) ? notes.images : [];
  const out = [];
  for (const entry of Array.isArray(notes?.entries) ? notes.entries : []) {
    const text = String(entry.text ?? '').trim();
    // 给某个泡泡补的图（forKey）挂在那个泡泡上，不属于任何一条手记
    const images = all.filter((im) => !im.forKey && im.entryId === entry.id);
    if (!text && !images.length) continue;
    const items = [];
    if (text) items.push({ ts: entry.ts ?? fallback, kind: 'note', label: text, sourceId: sourceId('note', `${localDate}#${entry.id}`) });
    ...
    out.push({
      id: `mod:note:${entry.id}`,
      key: noteKey(entry.id),
      ...
      score: 100,
      selected: true,
      why: '你自己写的记录，模型会优先采信；不想写进日记就戳破它，要彻底删掉就点「删掉这条手记」',
    });
  }
  return out;
}
...
export function buildModules(input, opts = {}) {
  ...
  const modules = [];
  const leftovers = [];
  // 手记排在最前：它是用户亲手写的，最该被先看到
  if (opts.localDate) modules.push(...notesModules(input.notes, opts.localDate));
