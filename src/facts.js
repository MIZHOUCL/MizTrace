...
/** 手记 → 证据行：文字一行（source_ref = 日期）、每张图一行（source_ref = 图片 id，path 是文件）。 */
export function evidenceFromNotes(notes, localDate, cutoffHour) {
  ...
  if (text) {
    const ts = notes.updatedAt ?? fallback;
    rows.push({ source_type: 'note', source_ref: localDate, project_id: 'notes', occurred_at: ts, local_date: localDate, level: 'L1', excerpt: text });
  }
