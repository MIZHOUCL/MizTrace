...
export const DEFAULTS = {
  ...
  /**
   * SVN 工作副本采集。默认关闭 —— 因为 svn log 是**向版本库服务器发起的查询**，
   * 不像 git log / git status 全在本地，打开它就意味着采集时会有一次出站请求。
   * 开了以后也只读 revision / 作者 / 时间 / 提交说明 / 改动路径，不读 diff 正文。
   * remote: false 可以只采集本地的 status（未提交改动）与 info，完全不碰服务器。
   */
  svn: {
    enabled: false,
    remote: true,
    /** 在扫描目录下找工作副本的深度，与 git 保持一致。 */
    maxDepth: 4,
    /** 单次 svn log 最多取多少个 revision，防止一个巨大的仓库拖慢采集。 */
    maxRevisions: 500,
  },
...
/** 合并前端提交的 SVN 配置。 */
export function mergeSvnConfig(current, incoming) {
  const next = { ...DEFAULTS.svn, ...(current ?? {}) };
  if (!incoming || typeof incoming !== 'object') return next;
  if (typeof incoming.enabled === 'boolean') next.enabled = incoming.enabled;
  if (typeof incoming.remote === 'boolean') next.remote = incoming.remote;
  if (Number.isInteger(incoming.maxDepth) && incoming.maxDepth >= 1) next.maxDepth = Math.min(10, incoming.maxDepth);
  if (Number.isInteger(incoming.maxRevisions) && incoming.maxRevisions >= 1) next.maxRevisions = Math.min(5000, incoming.maxRevisions);
  return next;
}
...
export function loadConfig() {
  ...
  cfg.svn = { ...DEFAULTS.svn, ...(onDisk.svn || {}) };
