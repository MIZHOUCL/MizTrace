...
test('给泡泡补的图挂到那个模块上，手记模块不显示它；浏览段的键精确到分钟，撞车加 #2', async () => {
  const { attachHintImages, dedupeKeys, notesModule } = await import('../src/modules.js');
  const visits = [1, 2, 3].map((i) => ({ ts: `2026-09-07T02:0${i}:00.000Z`, url: `https://x.com/${i}`, host: 'x.com', title: `p${i}`, secs: 10 }));
  const notes = { text: '', updatedAt: null, images: [{ id: 'im1', name: 'a.png', mime: 'image/png', path: '/x/a.png', ts: '2026-09-07T03:00:00Z', forKey: 'web@2026-09-07T02:01' }] };
  ...
  assert.equal(notesModule(notes, '2026-09-07'), null, '只有配图、没有手记文字：没有手记模块');
