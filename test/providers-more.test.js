...
test('CodeBuddy / WorkBuddy：认不出的格式依次退回 Claude Code、通用格式；都标通用', () => {
  const home = tmp();
  ...
  // WorkBuddy 已经拿到真实样例、拆成专用适配器，所以不再是「通用格式」；CodeBuddy 仍是
  assert.ok(wb && cb, '两个都在注册表里');
  assert.ok(!wb.generic, 'WorkBuddy 有专用适配器（2026-09-15 拿到真实会话文件）');
  assert.ok(cb.generic, 'CodeBuddy 还没拿到样例，仍标通用');
...
test('WorkBuddy 专用格式：input_text/output_text 是正文，function_call 记动作，ai-title 当标题，思考与工具输出不读', () => {
  const home = tmp();
  const proj = path.join(home, 'projects', 'e-DayTrace');
  fs.mkdirSync(proj, { recursive: true });
  const line = (o) => `${JSON.stringify(o)}\n`;
  // cwd / 文件路径按运行平台取写法：CI 是 macOS + Windows 双平台，isAbsolute 在两边认的东西不一样
  const win = process.platform === 'win32';
  const cwd = win ? 'e:\\DayTrace' : '/work/app';
  const file = win ? 'e:\\DayTrace\\src\\day.js' : '/work/app/src/day.js';
  const base = { sessionId: 's-1', cwd, providerData: {}, id: 'x' };
  fs.writeFileSync(
    path.join(proj, 'd52f7c15-c2c7-4d7f-909b-9c8af2aa308d.jsonl'),
    line({ ...base, type: 'message', role: 'user', timestamp: T(9), content: [{ type: 'input_text', text: '帮我给项目加一个 SVN 采集' }] }) +
      line({ ...base, type: 'reasoning', timestamp: T(9, 1), content: '用户想要 SVN 支持', rawContent: '用户想要 SVN 支持' }) +
      line({ ...base, type: 'function_call', name: 'Read', timestamp: T(9, 2), arguments: JSON.stringify({ file_path: file, offset: 1, limit: 20 }) }) +
      line({ ...base, type: 'function_call', name: 'Edit', timestamp: T(9, 3), arguments: JSON.stringify({ file_path: file, old_string: 'a', new_string: 'b' }) }) +
      line({ ...base, type: 'function_call', name: 'Bash', timestamp: T(9, 4), arguments: JSON.stringify({ command: 'node --test', description: '跑一遍测试' }) }) +
      line({ ...base, type: 'function_call', name: 'WebSearch', timestamp: T(9, 5), arguments: JSON.stringify({ query: 'svn log xml 日期范围' }) }) +
      line({ ...base, type: 'function_call_result', name: 'Edit', timestamp: T(9, 6), status: 'completed', output: { 不该被读到: true }, callId: 'c1' }) +
      line({ ...base, type: 'message', role: 'assistant', timestamp: T(9, 7), content: [{ type: 'output_text', text: '加好了，这个来源默认关闭。' }] }) +
      line({ ...base, type: 'ai-title', timestamp: T(9, 8), aiTitle: '给项目加 SVN 采集' }) +
      '{ 坏行\n',
  );
  const wb = PROVIDERS.find((p) => p.id === 'workbuddy');
  const out = wb.mod.collect([home], RANGE, 4, {});
  assert.equal(out.length, 1, '一个文件一个会话；坏行不该让整个会话消失');
  const s = out[0];
  assert.equal(s.sessionId, 's-1', '用记录里的 sessionId，不用文件名');
  assert.equal(s.cwd, cwd);
  assert.equal(s.title, '给项目加 SVN 采集', 'ai-title 就是会话标题');
  assert.deepEqual(s.prompts.map((p) => p.text), ['帮我给项目加一个 SVN 采集']);
  assert.deepEqual(
    s.actions.map((a) => [a.kind, a.value]),
    [
      ['file', file],
      ['command', '跑一遍测试'],
      ['search', 'svn log xml 日期范围'],
    ],
    'Read 这类只读工具不记；Bash 记它给的 description',
  );
  assert.equal(s.replies.length, 1);
  assert.match(s.replies[0].text, /^加好了/);
  assert.ok(!JSON.stringify(s).includes('不该被读到'), 'function_call_result 里的工具输出不读');
  assert.ok(!JSON.stringify(s).includes('用户想要 SVN 支持'), 'reasoning 里的思考过程不读');
});

test('WorkBuddy 只进 projects 目录，不去读 sessions / logs 那些非对话文件', () => {
  const home = tmp();
  fs.mkdirSync(path.join(home, 'sessions'), { recursive: true });
  fs.mkdirSync(path.join(home, 'logs'), { recursive: true });
  // 运行时心跳状态：字段齐全但不是对话，不该被当会话
  fs.writeFileSync(
    path.join(home, 'sessions', '12804.json'),
    JSON.stringify({ pid: 12804, sessionId: 'x', cwd: '/work/app', startedAt: T(9), lastHeartbeat: T(9, 5), kind: 'chat' }),
  );
  fs.writeFileSync(path.join(home, 'logs', 'run.jsonl'), `${JSON.stringify({ type: 'message', role: 'user', timestamp: T(9), content: [{ type: 'input_text', text: '日志里的东西不该进日记' }] })}\n`);
  const wb = PROVIDERS.find((p) => p.id === 'workbuddy');
  // projects 目录不存在时退回扫整个目录，所以这里先建一个空的 projects
  fs.mkdirSync(path.join(home, 'projects'), { recursive: true });
  assert.deepEqual(wb.mod.collect([home], RANGE, 4, {}), [], 'projects 下没有 jsonl 就没有会话');
});
